...
(function() {
  ...
  UserStatusController = Em.ObjectController.extend({
    init: function() {
      ...
      this.auth.on("signInError", function() {
        console.debug("failure callback in user status controller");
        return _this.set("flash", "Sign In Failed");
      });
      return this.auth.on("signInSuccess", function() {
        console.debug("failure callback in user status controller");
...
(function() {
  Em.Auth.Request.MyDummy = Em.Object.extend({
    validCreds: function(email, password, opts) {
      console.debug("email: " + email + " password: " + password);
      return opts.store.find('user').then(function(users) {
        var found;
    ...
    signIn: function(url, opts) {
      ...
      console.mylog(opts);
      return this.validCreds(opts.data.user.email, opts.data.user.password, {
        store: opts.store,
        success: function(user) {
          console.debug("is valid");
          _this.auth.trigger('signInSuccess');
          _this.auth.set('user', user);
          return _this.auth.trigger('signInComplete');
        },
        failure: function() {
          console.debug("is not valid");
          _this.auth.trigger('signInError');
          return _this.auth.trigger('signInComplete');
        }
      });
    },
    signInx: function() {
      return this.auth.trigger('signInError');
    },
...
// Generated by CoffeeScript 1.6.3
(function() {
  var auth, baseObjs, getBaseObjs, k, setupEmberInit, v;
...
},{"./auth_setup":1,"./controllers/register":2,"./controllers/sign_in":3,"./controllers/user_status":4,"./dummy_request":5,"./models/user":7,"./routes/register":8,"./templates":9,"./test_helpers":10}],7:[function(require,module,exports){
// Generated by CoffeeScript 1.6.3
(function() {
  var Appx;
...
Em.TEMPLATES['_user_status'] = Em.Handlebars.compile('<span class="user-status">   <span class="user-flash">     {{flash}}   </span>   {{#if auth.signedIn}}     <span class="signed-in">       Signed In as {{auth.user.email}}     </span>     {{render sign_out}}   {{else}}     <span class="signed-out">       {{render sign_in}}     </span>   {{/if}} </span>');

Em.TEMPLATES['-user-status'] = Em.Handlebars.compile('<span class="user-status">   <span class="user-flash">     {{flash}}   </span>   {{#if auth.signedIn}}     <span class="signed-in">       Signed In as {{auth.user.email}}     </span>     {{render sign_out}}   {{else}}     <span class="signed-out">       {{render sign_in}}     </span>   {{/if}} </span>');

Em.TEMPLATES['user_status'] = Em.Handlebars.compile('<span class="user-status">   <span class="user-flash">     {{flash}}   </span>   {{#if auth.signedIn}}     <span class="signed-in">       Signed In as {{auth.user.email}}     </span>     {{render sign_out}}   {{else}}     <span class="signed-out">       {{render sign_in}}     </span>   {{/if}} </span>');
...
(function() {
  ...
  helpers.loginFail = function() {
    return helpers.loginWith("user@fake.com", "passwordwrong" + (Math.random() * 10000000000));
  };
